...
public class buildPyramid//I struggled hard because of WriteLine. I needed to use Write instead so I wouldnt end up with 122333444 etc.
{
	...
	{
		for (int i = 1; i <= rows; i++)// will write out rows
		{
...
The Second "for" is writen in the body of the first "for" and will populate our rows with the columns value. (int named fill has a value of 1; if fill is less than or equal to i(which holds whichever current value it has); increase the value of fill by one) (So the 5th Colummn would have a row of 55555).
